...
\subsubsection{Représentation des maillages tétraèdriques}
\label{representation_maillages_tetra}
\noindent
Ce rapport se concentre sur les structures de données pour maillages tétraèdriques. Comme rappelé dans l'introduction, une structure de données doit permettre une certaine utilisation du maillage. Voici certaines des opérations qu'elle doit supporter :\\
...
\end{itemize}
Au total cela fait donc 8 références par tétraèdre et 1 référence par sommet (8$|$T$|$+$|$V$|$). Avec ces références, on peut mettre au point des algorithmes pour répondre à toutes les requêtes ci-dessus.