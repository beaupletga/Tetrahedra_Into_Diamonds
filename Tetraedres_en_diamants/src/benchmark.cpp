...
void time_to_access_ith_diamond(bool diamond_extra_bytes_array[],int array_size,int diamond_number)
{
    ...
    write_in_csv(values,"time_to_access_ith_diamond2.csv");
}
...
void time_to_access_ith_tetra(bool diamond_extra_bytes_array[],int array_size,int tetra_number)
{
    ...
    write_in_csv(values,"time_to_access_ith_tetra2.csv");
}
...
vector<tuple<int,int,int>> &face_array,int vertex_number)
{
    ...
    write_in_csv(values,"time_to_compute_vertex_degree2.csv");
}
...
vector<tuple<int,int,int>> &face_array,vector<Tetrahedron> &tetra_list,int diamond_number)
{
    double count_time=0;
    vector<double> values;
    for (int i=0;i<500;i++)
    {
        ...
        auto duration = chrono::duration_cast<chrono::microseconds>(stop - start); 
        count_time+=duration.count();
        values.push_back(duration.count()/(double)tetra_list.size());
    }
    cout<<"Average time to compute BFS : "<<count_time*1e-6/500<<" s"<<endl;
    write_in_csv(values,"time_to_compute_BFS2.csv");
}
